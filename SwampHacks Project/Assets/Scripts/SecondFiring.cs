...
public class SecondFiring : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if (Input.GetKey (KeyCode.S) && coolDownTimer <= 0) {
			...
			SecondBullet bulletScript2 = bullet.GetComponent<SecondBullet>();
		...
		if (Input.GetKey (KeyCode.K) && coolDownTimer <= 0) {
			...
			SecondBullet bulletScript2 = bullet2.GetComponent<SecondBullet>();
